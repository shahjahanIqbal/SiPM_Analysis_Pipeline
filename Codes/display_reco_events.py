...
# Create button axes first and store direct references — never touch these
...
# Protected axes — never removed
PROTECTED = {ax_btn_next, ax_btn_skip, ax_btn_exit}


def clear_axes():
    """Remove every axes that is not a button axes."""
    for ax in fig.axes[:]:          # copy the list — we're modifying it
...
def draw_event(event):
    ...
    ax_cam.set_title(f"Run {obs_id} | Event {event_id} | Tel {tel_id}",
    ...
    if hillas is not None and np.isfinite(hillas.length.value):
        ...
        ax_cam.add_patch(Ellipse(
            xy=(x, y), width=2 * length, height=2 * width,
            angle=np.degrees(psi),
            edgecolor='#e77e51', facecolor='none',
            linewidth=1.8, linestyle='--', zorder=5,
        ))
        ax_cam.plot(x, y, '+', color='#e77e51',
                    markersize=10, markeredgewidth=2, zorder=6)
        ax_cam.plot([x - dx, x + dx], [y - dy, y + dy],
                    color='#e77e51', linewidth=1.4, zorder=5)
...
def on_next(click):
    try:
        ...
        ax.text(0.5, 0.5, "End of file", ha='center', va='center',
